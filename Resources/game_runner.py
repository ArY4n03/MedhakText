...
class GameRunner:
    ...
    def GameScreenContent(self):
        ...
        #EndScreen Content
            ...
            self.SaveBtn = tk.Button(self.EndScreen,text="Save Story",font=(None,30),command=self.print_story)
            self.SaveBtn.place(relx=0.38,rely=0.7,relheight=0.1,relwidth=0.2)
    ...
    def get_sceneFile_data(self,scene_file):
        with open("Scenes/"+scene_file,'r') as file:
            content = file.read()
    ...
    def startGame(self):
        self.scene_file = self.settings["start_scene"]
        self.change_frame()
